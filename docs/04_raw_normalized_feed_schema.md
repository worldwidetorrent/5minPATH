...
- a sanctioned phase-1 capture workflow now materializes real files under `data/raw/...` and `data/normalized/...` via `./scripts/run_collectors.sh`, either as a one-shot pass, a coarse bounded smoke session, or a denser pilot/admission session with source-specific cadence
...
- the bounded capture path now records per-sample diagnostics under `artifacts/collect/.../sample_diagnostics.jsonl`, writes crash-safe incremental checkpoints under `artifacts/collect/.../summary.partial.json`, incrementally appends the raw and normalized session partitions during the run, persists explicit lifecycle states (`running`, `degraded`, `completed`, `failed_cleanly`, `aborted_watchdog`, `aborted_source_failure`), writes replay-admission rollups under `artifacts/collect/.../admission_summary.json`, retries transient external fetches with bounded backoff plus jitter, classifies retryable DNS / URL / timeout / HTTP failures explicitly, uses a forward-progress watchdog and Chainlink circuit breaker to avoid zombie runtime, treats empty Polymarket books as degraded capture states rather than fatal exceptions, uses rollover-aware Polymarket selector refresh before declaring a market binding invalid, and judges family continuity from the final selected market/window state per sample rather than from the full refreshed metadata strip
- the current Polymarket quote path now distinguishes `valid_empty_book`, `quote_unavailable`, and `binding_invalid` semantics, and both `summary.json` and `admission_summary.json` now report per-window quote coverage so thin but valid books can be separated from true source/binding failures
