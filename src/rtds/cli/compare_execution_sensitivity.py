"""Stress replay regimes under stricter execution assumptions for one capture session."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir = run_execution_sensitivity_comparison(args)
    print(run_dir)
    return 0


def run_execution_sensitivity_comparison(args: argparse.Namespace) -> Path:
    """Run the default degraded-regime execution sensitivity matrix."""

    trade_date = date.fromisoformat(args.date)
    if not isinstance(args.session_id, str) or not args.session_id.strip():
        raise ValueError("--session-id is required for execution sensitivity comparison")
    ...
    variant_results = tuple(
        ...
        for variant in DEFAULT_EXECUTION_VARIANTS
    )
    write_json_file(
        run_dir / "sensitivity_summary.json",
        {
            "trade_date": trade_date.isoformat(),
            "session_id": session_id,
            "admission_summary_path": str(admission_summary_path),
            "variants": [execution_variant_result_to_dict(item) for item in variant_results],
        },
    )
    for variant_result in variant_results:
        variant_dir = run_dir / "variants" / variant_result.variant_name
        write_json_file(
            variant_dir / "summary.json",
            execution_variant_result_to_dict(variant_result),
        )
