"""Compare replay outcomes across window-aware policy stacks for one capture session."""
...
from rtds.replay.policy_stack import (
...
DEFAULT_STACK_PATHS: tuple[str, ...] = (
    "configs/replay/policy_stacks/baseline_only.yaml",
    "configs/replay/policy_stacks/baseline_plus_degraded_light.yaml",
    "configs/replay/policy_stacks/baseline_plus_degraded_light_gated_medium.yaml",
)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    run_dir = run_policy_stack_comparison(args)
    print(run_dir)
    return 0


def run_policy_stack_comparison(args: argparse.Namespace) -> Path:
    trade_date = date.fromisoformat(args.date)
    if not isinstance(args.session_id, str) or not args.session_id.strip():
        raise ValueError("--session-id is required for policy stack comparison")
    ...
    stacks = tuple(load_policy_stack(path) for path in args.stack_config)
    results = tuple(
        build_policy_stack_result(
            evaluation_rows,
    ...
    write_json_file(
        run_dir / "policy_stack_summary.json",
        {
            "trade_date": trade_date.isoformat(),
            "session_id": session_id,
            "admission_summary_path": str(admission_summary_path),
            "stacks": [policy_stack_result_to_dict(result) for result in results],
        },
    )
    write_text_file(
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--stack-config",
        action="append",
        default=list(DEFAULT_STACK_PATHS),
        help="Path to one policy-stack config. Repeat to override the default stack set.",
    )
