...
def test_state_assembler_marks_non_normalized_venue_invalid_for_composite() -> None:
    assembler = CaptureOutputStateAssembler(session_id="20260326T010000000Z")
    assembler.ingest_chainlink_row(
        {
            "event_id": "chain-1",
    ...
    assert state.exchange_quote_valid_for_composite_by_venue["binance"] is False
    assert (
        state.exchange_quote_invalid_reason_by_venue["binance"]
        == "normalization_status_excluded"
    )
    assert state.exchange_eligible_by_venue["binance"] is False
    assert (
        state.exchange_ineligible_reason_by_venue["binance"]
        == "normalization_status_excluded"
    )
    assert (
    ...
    assert state.exchange_trusted_venue_count == 2
    assert state.state_invalid_reason == NoTradeReason.INSUFFICIENT_TRUSTED_VENUES
