...
- The current degraded-regime analysis tranche now freezes both the 6-hour baseline and the 12-hour pilot under one shared contract at [`docs/baselines/task7_reference_inputs.md`](/home/ubuntu/testingproject/docs/baselines/task7_reference_inputs.md), with a machine-readable manifest in [`configs/baselines/analysis/task7_reference_runs.json`](/home/ubuntu/testingproject/configs/baselines/analysis/task7_reference_runs.json) and a fixed replay config in [`configs/replay/task7_reference_comparison.yaml`](/home/ubuntu/testingproject/configs/replay/task7_reference_comparison.yaml). Reason: Task 7 now needs reproducible economic comparisons across pinned sessions, not ad hoc reruns with drifting configs.
- The degraded-window classifier is now explicit and versioned in [`configs/replay/window_quality_classifier_v1.json`](/home/ubuntu/testingproject/configs/replay/window_quality_classifier_v1.json), and capture admission emits that classifier contract beside every per-window verdict table. Reason: Task 7 needs stable `good` / `degraded_light` / `degraded_medium` / `degraded_heavy` / `unusable` labels that can be reapplied consistently across the 6-hour and 12-hour reference runs.
- The canonical replay runner now accepts `--session-id` for session-scoped rebuilds. Reason: pinned baseline sessions must be replayed against their own normalized partitions, not against an accidental union of every capture under the same UTC date.
...
- Admission semantics are now `v2`: session-level continuity and snapshot eligibility determine whether a run is structurally usable, while window-level verdicts determine what can actually enter replay and policy extraction. The old blunt degraded-count result is preserved as `legacy_verdict` for comparison. Reason: the 6-hour and 12-hour pinned sessions both proved that session-wide rejection was too blunt once family continuity, oracle continuity, exchange continuity, and snapshot eligibility were already strong.
- The first window-aware policy stack is now encoded in replay configs: baseline `good` windows only, exploratory `degraded_light` overlay, and tightly gated `degraded_medium` overlay for `large_positive_edge`, `mid/high_vol`, and `wide_spread` slices. Reason: policy-stack replay on the pinned 6-hour and 12-hour sessions confirmed that the good-window baseline remains the clean extraction universe, while degraded overlays add narrower exploratory flow rather than a new merged default universe.
- The current merge-ready semantic freeze is recorded in [`docs/decisions/0005_policy_v1_and_admission_v2.md`](/home/ubuntu/testingproject/docs/decisions/0005_policy_v1_and_admission_v2.md). Reason: the branch has now earned promotion, so `policy v1` and `admission semantics v2` need one explicit decision record rather than only scattered baseline notes.
- Long-running capture sessions are now crash-safe at the session-artifact layer: `sample_diagnostics.jsonl` appends as samples complete and `summary.partial.json` is checkpointed during the run so a late failure can still be diagnosed from durable partial artifacts. Reason: the next validation step is 24-hour durability, so long sessions must either complete cleanly or fail with usable partial session state.
